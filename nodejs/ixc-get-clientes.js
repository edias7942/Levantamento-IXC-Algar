...
let options = {
  method: "GET",
  url: "https://ixc.opponet.com.br/webservice/v1/cliente",
  headers: {
...
request(options, function (error, response, body) {
  ...
  // Criação da Tabela
  let create_new_table = false;
  let create_new_table_command = "CREATE TABLE IF NOT EXISTS clientes (";
  ...
  for (let registro of registros) {
    let add_new_client_command = "INSERT INTO clientes VALUES (";
    ...
    db.run(add_new_client_command, function(err) {
      if (err) {
        ...
        console.log(`Usuário ${registro.razao} inserido com sucesso, ID: ${this.lastID}`);
      }
    });
  }
});
